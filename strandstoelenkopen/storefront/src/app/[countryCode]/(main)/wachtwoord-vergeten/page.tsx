...
export default function RequestResetPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    fetch("/api/wachtwoordreset", {
      ...
      .then((res) => {
        if (res.ok) {
          setMessage("Indien account met emailadres bestaat, is de reset link verstuurd naar het emailadres.")
        } else {
          setMessage("Reset link kan niet worden verstuurd. Neem contact op via info@richolland.nl")
        }
      })
      .catch((error) => {
        alert(error.message)
  ...
  return (
    ...
    >
      ...
      ${
        loading
          ? "w-full mt-6 bg-gray-900 hover:bg-gray-800"
          : "w-full mt-6 bg-gray-900 hover:bg-gray-800"
      }
    `}
      >
      ...
      {message && <p className="bg-gray-900 hover:bg-gray-800 p-4 my-6 border-gray-600 rounded-lg text-white">{message}</p>}
