...
const MainDescriptionWrapper = ({
  ...
  const { data: queryResult } = useQuery({
    ...
    queryFn: () => sdk.admin.productCollection.retrieve(productCategory.id),
  });
  ...
  return (
    <MainDescriptionForm
      ...
      metadata={queryResult.collection.metadata || {}}
    />
  );
};
...
const MainDescriptionForm = ({ productCategoryId, metadata }: Props) => {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: CustomFields) =>
      sdk.admin.productCollection.update(productCategoryId, {
